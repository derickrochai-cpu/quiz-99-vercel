...
async function getGame(gameCode) {
  ...
  if (isSupabaseEnabled && supabase) {
    ...
    const game = {
      ...
      questionStartedAt: data.question_started_at,  // Novo campo para timer
...
async function updateGame(gameCode, updates) {
  ...
  if (isSupabaseEnabled && supabase) {
    ...
    // Só adiciona question_started_at se foi fornecido
    if (updates.questionStartedAt) {
      dbUpdates.question_started_at = updates.questionStartedAt;
    }
    ...
    console.log('[updateGame] Updating game:', gameCode, 'with fields:', Object.keys(dbUpdates));
