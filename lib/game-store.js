...
async function updateGame(gameCode, updates) {
  // Flag para controlar se deve atualizar o timestamp (usado em respostas de jogadores)
  const shouldUpdateTimestamp = !updates.skipTimestampUpdate;

  // Remove a flag antes de processar
  delete updates.skipTimestampUpdate;

  // Atualiza em memória primeiro
  const existingGame = memoryGames.get(gameCode);
  if (existingGame) {
    Object.assign(existingGame, updates);
  ...
  if (isSupabaseEnabled && supabase) {
    const dbUpdates = {
      ...
      question_started_at: updates.questionStartedAt,
      started_at: updates.startedAt ? new Date(updates.startedAt).toISOString() : undefined,
      results: updates.results
    };

    // Só atualiza o timestamp se necessário
    if (shouldUpdateTimestamp) {
    ...
    console.log('[updateGame] Updating game:', gameCode, 'with fields:', Object.keys(dbUpdates), 'updateTimestamp:', shouldUpdateTimestamp);
    ...
    if (error) {
      ...
      console.log('[updateGame] Jogo atualizado com sucesso:', gameCode);
    }
  }
